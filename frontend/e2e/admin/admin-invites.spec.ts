...
test.describe('AdminInvitesPage', () => {
  ...
  test('rejects inviting an email that already belongs to a member (#64)', async ({
    page,
  }) => {
    await page.goto('/admin/invites');
  ...
  test('revokes an unused (sent) invite', async ({ page }) => {
    await page.goto('/admin/invites');
  ...
  test('only sent invites show a Revoke action', async ({ page }) => {
    await page.goto('/admin/invites');
